...
npm install
cp .env.example .env
# Edit CHARTDB_SYNC_PASSWORD in .env
npm run dev
```

Open `http://127.0.0.1:5173` and enter the password from `.env`. The first
browser that already contains diagrams initializes `data/chartdb-sync.json`.
Other browsers connected to the same service import that workspace
automatically. The default sync interval is 10 seconds and can be changed with
`CHARTDB_SYNC_INTERVAL_MS`.
...
```bash
docker build -t chartdb-sync .
docker run \
  -e CHARTDB_SYNC_PASSWORD=<YOUR_PASSWORD> \
  -e OPENAI_API_KEY=<YOUR_OPEN_AI_KEY> \
  -v chartdb-data:/usr/src/app/data \
  -p 8080:80 \
  chartdb-sync
```
...
For a public VPS, set `HOST=0.0.0.0`, keep
`CHARTDB_SYNC_FILE` on persistent storage, and place the service behind an
HTTPS reverse proxy. One Node process must own one workspace file.
