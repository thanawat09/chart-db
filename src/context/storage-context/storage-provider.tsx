...
export const StorageProvider: React.FC<React.PropsWithChildren> = ({
    ...
    const getSyncMetadata: StorageContext['getSyncMetadata'] = useCallback(
        async () => await db.sync_metadata.get(1),
        [db]
    );

    const updateSyncMetadata: StorageContext['updateSyncMetadata'] =
        useCallback(
            async (metadata) => {
    ...
    const applyWorkspaceSync: StorageContext['applyWorkspaceSync'] =
        useCallback(
            async ({
                diagrams,
                deletedDiagramIds,
                ...
                await db.transaction(
                    ...
                    async () => {
                        const deleteStoredDiagram = async (
                            diagramId: string
                        ) => {
                            await Promise.all([
                        ...
                        for (const diagramId of deletedDiagramIds) {
                            await deleteStoredDiagram(diagramId);
                            await db.diagram_filters.delete(diagramId);
                        }

                        for (const diagram of diagrams) {
                            await deleteStoredDiagram(diagram.id);
                            ...
                            await Promise.all([
                                db.db_tables.bulkPut(
                                    (diagram.tables ?? []).map((table) => ({
                                        ...table,
                                        diagramId: diagram.id,
                                    }))
                                ),
                                db.db_relationships.bulkPut(
